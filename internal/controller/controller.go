...
func (c *Controller) Ready() error {
    // mountpoint exists and writable
...
func (c *Controller) ensureMounter() error {
    ...
    // Optional network aliases to target node-local LB endpoint
    ...
    if c.cfg.EnableProxy && c.cfg.LocalLBEnabled && strings.TrimSpace(c.cfg.ProxyNetwork) != "" {
        ...
        netCfg = &network.NetworkingConfig{EndpointsConfig: map[string]*network.EndpointSettings{
            c.cfg.ProxyNetwork: {Aliases: []string{alias}},
        }}
    } else {
...
func (c *Controller) buildRcloneEnv() []string {
    ...
    env := []string{
        ...
        fmt.Sprintf("RCLONE_CONFIG_S3_ACCESS_KEY_ID=%s", strings.TrimSpace(string(accessKey))),
        fmt.Sprintf("RCLONE_CONFIG_S3_SECRET_ACCESS_KEY=%s", strings.TrimSpace(string(secretKey))),
...
func (c *Controller) localAlias() string {
    return "swarm-s3-mounter-lb-" + sanitizeHostname()
}
